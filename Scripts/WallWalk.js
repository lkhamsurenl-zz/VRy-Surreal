#pragma strict
 ...
 var walkUpRange: float = 0.1;
 var eyecamera : GameObject;
 var SimulationRate: float = 60f;
...
/*
private var ovr : OVRCameraRig;
 function Awake () {
 	var camera = GameObject.Find("Camera");
 	ovr = camera.GetComponent("OVRCameraRig");
 }
*/ 
 ...
 function Start(){
     ...
     var playerCollider = gameObject.GetComponent(CapsuleCollider);
     distGround = playerCollider.bounds.extents.y - playerCollider.center.y;
     eyecamera = GameObject.Find("Player/Capsule/Camera/CenterEyeAnchor");
     
     //var ovr = theCamera.GetComponents(OVRCamerRig);
    //print(ovr.centerEyeAnchor);
 }
 
 function FixedUpdate(){
     // apply constant weight force according to character normal:
     rigidbody.AddForce(-gravity*rigidbody.mass*myNormal, ForceMode.VelocityChange);
 }
 ...
 function Update(){
     ...
     if (Input.GetKeyDown("j")){ // j enables "jump":
 ...
 function RotateTo(targetRot:Quaternion){
 	var i = 0.0;
	var rate = 1.0/2.0;
	while (i < 1.0){
		i+= Time.deltaTime*rate;
		transform.rotation = Quaternion.Slerp(transform.rotation, targetRot, i);
		//transform.rotation = Vector3.Lerp(transform.rotation, targetRot, i);
		yield;
	}
 }
